    ...
    <div className="space-y-8 p-1 md:p-4 max-w-7xl mx-auto print-container">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-4 border-b border-amber-200 pb-4 md:pb-5">
        ...
        <div className="flex items-center gap-2 md:gap-3 print-hide flex-col sm:flex-row w-full md:w-auto">
          <button
            ...
            className="hidden md:inline-flex items-center justify-center px-4 py-2 bg-amber-100 hover:bg-amber-200 text-amber-800 font-semibold rounded-xl text-sm transition-all duration-200 flex-1 md:flex-initial"
          ...
          <button
            ...
            className="px-5 py-2.5 bg-gradient-to-r from-amber-700 to-amber-800 hover:from-amber-800 hover:to-amber-900 text-white font-bold rounded-xl text-sm shadow-md hover:shadow-lg transition-all active:scale-[0.98] flex-1 md:flex-initial"
          >
            🖨️ Cetak
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 print-card-grid">
        ...
        <div className="bg-gradient-to-br from-emerald-600 to-emerald-700 text-white rounded-2xl p-6 shadow-md border border-emerald-500 relative overflow-hidden print-card">
          <p className="text-xs font-bold text-emerald-100 uppercase tracking-widest print-label">
            <span className="emoji-print-hide">💰 </span>Total Omset
          </p>
        ...
        <div className="bg-gradient-to-br from-amber-600 to-amber-700 text-white rounded-2xl p-6 shadow-md border border-amber-500 relative overflow-hidden print-card">
          <p className="text-xs font-bold text-amber-100 uppercase tracking-widest print-label">
            <span className="emoji-print-hide">💵 </span>Setoran Cash
          </p>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border-2 border-amber-200 relative print-card">
          <p className="text-xs font-bold text-amber-600 uppercase tracking-widest print-label">
            <span className="emoji-print-hide">⏳ </span>Pending
          </p>
          <p className="text-2xl font-black mt-2 text-amber-900 leading-none print-value">{formatPrice(report.pendingRevenue)}</p>
          <p className="text-xs text-amber-600 mt-3 font-semibold print-hide">⚠️ {report.unpaidOrdersCount} pesanan belum bayar</p>
        </div>

        {/* Total Orders Volume */}
        <div className="bg-white rounded-2xl p-6 shadow-sm border-2 border-amber-200 relative print-card">
          <p className="text-xs font-bold text-gray-500 uppercase tracking-widest print-label">
            <span className="emoji-print-hide">📋 </span>Total Pesanan
          </p>
        ...
        <div className="lg:col-span-1 bg-white rounded-2xl border-2 border-amber-200 p-6 space-y-6 print-border">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center text-sm font-semibold">
                <span className="text-gray-700 flex items-center gap-1.5">
                  <span className="emoji-print-hide">💵 </span>Cash
                </span>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center text-sm font-semibold">
                <span className="text-gray-700 flex items-center gap-1.5">
                  <span className="emoji-print-hide">
                    <QrCodeIcon />
                  </span>
                  <span>QRIS</span>
                </span>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center text-sm font-semibold">
                <span className="text-gray-700 flex items-center gap-1.5">
                  <span className="emoji-print-hide">🏦 </span>Transfer
                </span>
                <div className="flex items-center gap-2">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border-2 border-amber-200 p-6 print-border">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 print-hide">
            ...
            <div className="flex gap-1.5 bg-amber-50 p-1.5 rounded-xl border border-amber-200">
              <button onClick={() => setSelectedStatus("all")} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${selectedStatus === "all" ? "bg-amber-800 text-white shadow-sm" : "text-amber-900 hover:bg-amber-200/50"}`}>
                  ...
                  filteredItems.map((item) => (
                    <tr key={item.id} className="hover:bg-amber-50/40 transition-colors">
                      <td className="py-3 font-bold text-amber-900">
                        {item.orderId}
                        <span className="block font-normal text-[10px] text-gray-500 md:hidden mt-0.5">
                          Meja {item.tableNumber} • {item.time}
                        </span>
                      </td>
                        ...
                        <span className="inline-flex items-center gap-1">
                          {item.paymentMethod?.toLowerCase() === "qris" ? (
                            <span className="inline-flex items-center gap-1 bg-amber-50 text-amber-950 font-bold px-2 py-0.5 rounded border border-amber-200 text-xs">
                              <span className="print-hide-column">
                                <QrCodeIcon />
                              </span>
                              <span>QRIS</span>
                            </span>
                      ...
                      <td className="py-3 text-center print-hide-column">
                        <span className={`inline-block text-[10px] px-2 py-0.5 rounded-full font-bold border ${item.status === "paid" ? "bg-emerald-50 text-emerald-700 border-emerald-100" : "bg-amber-50 text-amber-700 border-amber-100"}`}>
